...
export default function LoginPage() {
  ...
  // Real Google button mount point. prompt()/One Tap alone silently does
  // nothing when the browser has no FedCM session (signed out, third-party
  // sign-in blocked, or a previously-dismissed prompt in cooldown) — with no
  // error surfaced to the user. renderButton() is the officially supported
  // control that falls back to a real OAuth popup in that case, so it's the
  // actual clickable element the user sees (not a custom button + prompt()).
    ...
    <div className="ls" style={{ flex: 1 }}>
      <div className="lc">
        ...
        {demo ? (
          ...
          <>
            {/* Real Google button, rendered by GSI once the script loads (see useEffect above).
                Not a custom button + prompt() — prompt()-only silently no-ops when the browser
                has no FedCM session, while this real button reliably falls back to a popup. */}
            <div style={{ display: 'flex', justifyContent: 'center', minHeight: 44 }}>
              <div ref={googleBtnRef} style={{ display: gsiReady ? 'block' : 'none' }} />
              {!gsiReady && (
                <button className="btn btn-sc btn-lg btn-full" disabled style={{ gap: 10 }}>
